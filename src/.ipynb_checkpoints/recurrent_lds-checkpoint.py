...
class LDS(nn.Module):
    ...
    """
    def __init__(self, lds, M_phi_plus, M_phi_minus, M):
        """
        Args:
          lds: LDS instance with attributes A, B, C, h0 already set.
          M_phi_plus, M_phi_minus: spectral weights of shape [K, d_in, d_out].
          M: autoregressive parameter of shape [d_out, d_in, k_u]. (If k_u==0, AR is disabled.)
        """
        super(LDS, self).__init__()
        self.lds = lds
        self.M_phi_plus = M_phi_plus  # e.g., from stu.M_phi_plus
        self.M_phi_minus = M_phi_minus
        self.M = M  # AR parameter
