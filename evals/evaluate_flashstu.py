...
from Flash_STU_550M import MiniSTU, MiniSTUConfig
import lm_eval as evaluator
import os
os.environ["HF_ALLOW_CODE_EVAL"] = "1"
os.environ["TOKENIZERS_PARALLELISM"] = "false"

# Dynamically register MiniSTU
if isinstance(CONFIG_MAPPING, _LazyConfigMapping):
    CONFIG_MAPPING._extra_content["ministu"] = MiniSTUConfig
else:
    CONFIG_MAPPING.update({"ministu": MiniSTUConfig})

if isinstance(MODEL_MAPPING, _LazyAutoMapping):
    MODEL_MAPPING._extra_content[MiniSTUConfig] = MiniSTU
else:
    MODEL_MAPPING.update({MiniSTUConfig: MiniSTU})

if isinstance(MODEL_FOR_CAUSAL_LM_MAPPING, _LazyAutoMapping):
    MODEL_FOR_CAUSAL_LM_MAPPING._extra_content[MiniSTUConfig] = MiniSTU
else:
    MODEL_FOR_CAUSAL_LM_MAPPING.update({MiniSTUConfig: MiniSTU})
