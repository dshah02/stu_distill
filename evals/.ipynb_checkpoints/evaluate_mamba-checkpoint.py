...
from Mamba_561M import MiniMamba, MiniMambaConfig
import lm_eval as evaluator
import os
os.environ["HF_ALLOW_CODE_EVAL"] = "1"
os.environ["TOKENIZERS_PARALLELISM"] = "false"

# Dynamically register MiniMamba
if isinstance(CONFIG_MAPPING, _LazyConfigMapping):
    CONFIG_MAPPING._extra_content["minimamba"] = MiniMambaConfig
else:
    CONFIG_MAPPING.update({"minimamba": MiniMambaConfig})

if isinstance(MODEL_MAPPING, _LazyAutoMapping):
    MODEL_MAPPING._extra_content[MiniMambaConfig] = MiniMamba
else:
    MODEL_MAPPING.update({MiniMambaConfig: MiniMamba})

if isinstance(MODEL_FOR_CAUSAL_LM_MAPPING, _LazyAutoMapping):
    MODEL_FOR_CAUSAL_LM_MAPPING._extra_content[MiniMambaConfig] = MiniMamba
else:
    MODEL_FOR_CAUSAL_LM_MAPPING.update({MiniMambaConfig: MiniMamba})

print("Registered MiniMamba model and configuration.")
...
for task in tasks:
    ...
    try:
        ...
        eval_kwargs = dict(
            model="hf",
            model_args=(
                "pretrained=Hazan-Lab/Mamba_561M,"
        ...
        print(f"An error occurred while evaluating task '{task}': {e}")
